---
title: "LBD MTL subfield analysis"
author: "Jesse Cohen"
date: "2023-01-05"
...
```{r load packages}
libraries <- c("tidyverse", "lubridate", "here", "ggpubr")
lapply(libraries, require, character.only = TRUE)

```

```{r munge1 - querying add'l data}
# Unique IDs to Query for more date in INDD

# get list of unique INDDs from initial query
original_query <- original_INQuery_Output_2022.07.12_09.59.INQuery
...
write_csv(inddids_all, file = here("reports", "inddids_all.csv"))

#remove(inddids_all)
```

```{r munge2 - rename objects}
# Rename INQuery output
clinical <- get((ls(pattern = "^original_.*.INQuery$"))) %>% 
...
saveRDS(clinical3, file = here("clinical_clean.RDS"))

# import other objects 
dem_biomarkers <- get(ls(pattern = "^demographic_biomarker_autopsy_.*.INQuery$")) %>% 
  mutate(INDDID = as.character(INDDID),
         Education = na_if(Education, 999),
         PET_read = Clinical_Read,
         tau_abeta42_ratio = LuminexTTauAbetaRatio)

ashs <- ashs_all_output %>% 
  mutate(INDDID = as.character(INDDID)) %>% 
...
# mri metadata
mri_bids_data <- get(ls(pattern = "^mri_bids_.*.INQuery$")) %>% 
  mutate(INDDID = as.character(INDDID))
...
saveRDS(mri_bids_data_t1, here("mri_bids_data_t1.RDS"))

mri_bids_data_t2 <- mri_bids_data_2 %>% 
  filter(FlywheelAcquisitionMeasurement == 'T2')
         
# clinical testing
mmse <- get(ls(pattern = "^mmse_.*.INQuery$")) %>% 
  mutate(INDDID = as.character(INDDID))

moca <- get(ls(pattern = "^moca_.*.INQuery$")) %>% 
  mutate(INDDID = as.character(INDDID))

vlt <- get(ls(pattern = "^vlt_.*.INQuery$")) %>% 
  mutate(INDDID = as.character(INDDID))

saveRDS(vlt, file = here("vlt_raw.RDS"))

epworth <- get(ls(pattern = "^epworth_.*.INQuery$")) %>% 
  mutate(INDDID = as.character(INDDID))
...
```{r munge 6 - merge clinical/mri metadata w/ ASHS}
