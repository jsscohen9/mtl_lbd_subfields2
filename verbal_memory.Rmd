...
ashs_clean <- read_rds("objects/hippo_icv_adjusted.RDS") #changed to adjusted
...
p_long <- pvlt_ashs %>% 
  select(-p_1, `Immediate Recall` = p_total,
         `Delayed Recall` = p_recall,
         `Recognition Discrimination` = p_recog,
         `Hippocampal Volume` = both_hippocamp_adjusted) %>% 
...
p_hip$test_component %<>% fct_relevel("Immediate Recall", 
                                     "Delayed Recall",
                                     "Recognition Discrimination")

ggscatter(p_hip, x = "vol", y = "z_score",
